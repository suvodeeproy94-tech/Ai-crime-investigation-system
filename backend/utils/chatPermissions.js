...
const canRolesChat = (senderRole, receiverRole) => {
    ...
    if (senderRole === 'admin') return ['police', 'user'].includes(receiverRole)
    // This rule allows police to chat with admin and user.
    if (senderRole === 'police') return ['admin', 'user'].includes(receiverRole)
    // This rule allows user to chat only with police.
    if (senderRole === 'user') return ['police'].includes(receiverRole)
...
const getAllowedTargetRoles = (role) => {
    ...
    if (role === 'admin') return ['police', 'user']
    // This line returns role list for police.
    if (role === 'police') return ['admin', 'user']
    // This line returns role list for normal user.
    if (role === 'user') return ['police']
